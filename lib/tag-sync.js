...
// Strip fields the spec-push endpoint either ignores or rejects:
//   - workbookId          lives in the URL path, not the body
//   - url                 read-only
//   - documentVersion,
//     latestDocumentVersion  read-only revision counters
//   - ownerId, folderId   read-only ownership metadata
//   - createdBy, updatedBy,
//     createdAt, updatedAt  read-only audit fields
//   - _meta               illustrative-only field we add locally
// Confirmed empirically against PUT /v2/workbooks/{id}/spec — body kept must
// include `schemaVersion`, `name`, `pages`, `layout`.
...
// Sigma version tags must exist in the org's tag registry before they can be
// applied to a workbook version. POST /v2/tags creates one. If the tag
// already exists, the API returns 409 and we treat that as success.
//
// Color picked deterministically from a fixed palette (Sigma allows: cyan,
// grass, violet, plum, amber, bronze). `template` always gets `bronze`.
const TAG_COLORS = ['cyan', 'grass', 'violet', 'plum', 'amber'];
function pickColor(tagName) {
  if (tagName === 'template') return 'bronze';
  let h = 0;
  for (let i = 0; i < tagName.length; i++) h = (h * 31 + tagName.charCodeAt(i)) | 0;
  return TAG_COLORS[Math.abs(h) % TAG_COLORS.length];
}

async function ensureTagExists(client, tagName) {
  const url = `${client.apiBase}/v2/tags`;
  const body = { name: tagName, color: pickColor(tagName) };

  client.onEvent({ level: 'info', action: 'ensureTagExists', method: 'POST', url, body });

  if (client.dryRun) {
    return { dryRun: true, action: 'ensureTagExists', name: tagName };
  }
  ...
  if (res.ok) {
    const parsed = tryParseJson(text);
    client.onEvent({ level: 'info', action: 'ensureTagExists:created', name: tagName, response: parsed });
    return parsed;
  }
  // Sigma uses 400 EINVAL "Version tags with the same name cannot exist" for
  // duplicates; other APIs may use 409. Cover both.
  if (
    res.status === 409 ||
    /already exists|duplicate|unique|same name cannot exist/i.test(text)
  ) {
    client.onEvent({ level: 'info', action: 'ensureTagExists:already-exists', name: tagName });
    return { name: tagName, alreadyExisted: true };
  }
  client.onEvent({ level: 'error', action: 'ensureTagExists', status: res.status, response: text });
  throw new Error(`Create tag failed (${res.status}): ${text}`);
}
...
// All higher-level operations take the canonical workbook ID explicitly so
// the library has no global config — the request body (web UI) or a CLI flag
// (script) supplies it per call.

async function syncCustomer(client, workbookId, customerId) {
  ...
  await ensureTagExists(client, tag);
...
async function syncAllCustomers(client, workbookId) {
  const results = [];
  for (const customerId of Object.keys(CUSTOMER_CONFIG)) {
    try {
      const result = await syncCustomer(client, workbookId, customerId);
...
async function propagateTemplate(client, workbookId) {
  const template = await pushTemplate(client, workbookId);
  const customers = await syncAllCustomers(client, workbookId);
